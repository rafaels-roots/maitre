				...
				<div class="ui-corner-all custom-corners">
					...
					<div class="ui-body ui-body-a">
						...
						<form method="post" >
							...
							<input type="number" name="mes_codigo" id="mes_codigo" data-wrapper-class="ui-custom" disabled>
							
							<label for="mes_descricao">Descrição: </label>
							<input type="text" id="mes_descricao" name="mes_descricao" data-wrapper-class="ui-custom" maxlength="30" />
								...
								if($(this).attr('id') == "Gravar") 
									{
										...
										var wsql = f_storageready("sql_statments") == "Incluir" ?  $("#mes_descricao").val() + "®" +
																								   $("#mes_ativa").val() + "©" : "" ; 
						
										wsql = f_storageready("sql_statments") == "Alterar" ? $("#mes_descricao").val() + "®" +
																							  $("#mes_ativa").val() + "®" + 
																							  $("#mes_codigo").val() + "©" : wsql;
																					
										wsql = f_storageready("sql_statments") == "Excluir" ? $("#mes_codigo").val() + "©" : wsql;
									
										var waction = f_storageready("sql_statments") == "Incluir" ? "f_insert" : f_storageready("sql_statments") == "Alterar" ? "f_update" : "f_delete";
										
										$.getJSON('http://www.sisleq.com.br/maitre/admin/mes_processa.php?callback=?', {action: waction, banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"] , sql: wsql}, function(data)
											{
