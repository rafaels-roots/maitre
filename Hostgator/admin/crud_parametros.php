				...
				<div class="ui-corner-all custom-corners">
					...
					<div class="ui-body ui-body-a">
						...
						<form method="post" >
							...
							<input type="text" name="par_mensagemrecibo1" id="par_mensagemrecibo1" data-wrapper-class="ui-custom" maxlength="20" />
							
							<label for="par_mensagemrecibo2">Mensagem Recibo : [2] </label>
							<input type="text" name="par_mensagemrecibo2" id="par_mensagemrecibo2" maxlength="20" data-wrapper-class="ui-custom" />
							
							<label for="par_garconcomissao">Comissão do Garçon/100 : [%] </label>
							<input type="number" name="par_garconcomissao" id="par_garconcomissao" maxlength="5" data-wrapper-class="ui-custom" />
				...
				$("#crud_parametros").on("pageshow", function() 
					{
						...
						$.getJSON('http://www.sisleq.com.br/maitre/admin/par_processa.php?callback=?', {action: "f_select",	banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"]}, function(data)
							{
								if( data.length == 0  )
									{  
							...
							{		
								if($(this).attr('id') == "Gravar") 
									{	
										...
										var wsql = f_storageready("sql_statments") == "Incluir" ? $("#par_garconexclusivo").val() + "®" + 
																								  $("#par_observacaopedido").val() + "®" +
																								  $("#par_ordemproduto").val() + "®" + 
																								  $("#par_mensagemrecibo1").val() + "®" +
																								  $("#par_mensagemrecibo2").val() + "®" +
																								  $("#par_garconcomissao").val() + "®" +
																								  $("#par_horarioverao").val() + "©" : ""; 
																	
										wsql = f_storageready("sql_statments") == "Alterar" ? $("#par_garconexclusivo").val() + "®" + 
																							  $("#par_observacaopedido").val() + "®" +  
																							  $("#par_ordemproduto").val() + "®" +  
																							  $("#par_mensagemrecibo1").val() + "®" +  
																							  $("#par_mensagemrecibo2").val() + "®" + 
																							  $("#par_garconcomissao").val() + "®" + 
																							  $("#par_horarioverao").val()  + "©"	: wsql;
										
																				var waction = f_storageready("sql_statments") == "Incluir" ? "f_insert" : f_storageready("sql_statments") == "Alterar" ? "f_update" : "f_delete";
										
										$.getJSON('http://www.sisleq.com.br/maitre/admin/par_processa.php?callback=?',{action: waction, banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"] , sql: wsql}, function(data)
											{
