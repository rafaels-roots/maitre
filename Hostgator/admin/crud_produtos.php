				...
				<div class="ui-corner-all custom-corners">
					...
					<div class="ui-body ui-body-a">		
						<form method="post" >
							...
							<input type="number" name="pro_codigo" id="pro_codigo" data-wrapper-class="ui-custom" disabled>
							
							<label for="pro_descricao">Descrição : </label>
							<input type="text" id="pro_descricao" name="pro_descricao" data-wrapper-class="ui-custom" maxlength="60" />
						
							<label for="pro_valor">Valor : </label>
							<input type="number" id="pro_valor" name="pro_valor" maxlength="6" data-wrapper-class="ui-custom" />
							
							<label for="pro_ordem">Ordem : [0-9] </label>
							<input type="number" name="pro_ordem" id="pro_ordem" maxlength="4" data-wrapper-class="ui-custom">
				...
				$("#crud_produtos").on("pageshow", function() 
					{
						...
						$.getJSON('http://www.sisleq.com.br/maitre/admin/pro_processa.php?callback=?', {action: "f_select_grusto",	banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"]}, function(data)
							{
								var gru_default = 0;
								var sto_default = 0;
								var select_Grupos = "";
						...
						$("#crud_produtos #menuopcoes ul").on( "click", "li", function() 
							{			
								...
								if($(this).attr('id') == "Gravar") 
									{
										$.mobile.loading("show");
										if(ElementsValidation([ {"Id" : "pro_descricao", "Caracteres" : "1"}, {"Id" : "pro_valor", "Caracteres" : "1"}], "minlength") == false)
											{
										...
										var wsql = f_storageready("sql_statments") == "Incluir" ? $("#pro_descricao").val() + "®" +
																								 $("#pro_unidade").val() + "®" +
																								 $("#pro_valor").val() + "®" +
																								 $("#pro_ativo").val() + "®" +
																				 				 $("#pro_ordem").val() + "®" +
										...
										wsql = f_storageready("sql_statments") == "Alterar" ? $("#pro_descricao").val() + "®" +
																							  $("#pro_unidade").val() + "®" +
																							  $("#pro_valor").val() + "®" +
																							  $("#pro_ativo").val() + "®" +
																							  $("#pro_ordem").val() + "®" +
										...
										wsql = f_storageready("sql_statments") == "Excluir" ? $("#pro_codigo").val() + "©"	: wsql;
							
										var waction = f_storageready("sql_statments") == "Incluir" ? "f_insert" : f_storageready("sql_statments") == "Alterar" ? "f_update" : "f_delete";

										$.getJSON('http://www.sisleq.com.br/maitre/admin/pro_processa.php?callback=?', {action: waction, banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"], sql: wsql }, function(data)
											{
