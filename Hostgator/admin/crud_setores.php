				...
				<div class="ui-corner-all custom-corners">
					...
					<div class="ui-body ui-body-a">		
						<form method="post" >
							...
							<input type="number" name="sto_codigo" id="sto_codigo" data-wrapper-class="ui-custom" disabled>
							
							<label for="sto_descricao">Descrição : </label>
							<input type="text" id="sto_descricao" name="sto_descricao" data-wrapper-class="ui-custom" maxlength="30" />
				...
				$("#crud_setores").on("pageshow", function() 
					{
								...
								if($(this).attr('id') == "Gravar") 
									{
										...
										var wsql = f_storageready("sql_statments") == "Incluir" ? $("#sto_descricao").val() + "®" +
																								  $("#sto_ativo").val() + "©" : "" ; 
																	
										wsql = f_storageready("sql_statments") == "Alterar" ? $("#sto_descricao").val() + "®" +
																							  $("#sto_ativo").val() + "®" +
																							  $("#sto_codigo").val() + "©" : wsql;
																					
										wsql = f_storageready("sql_statments") == "Excluir" ? $("#sto_codigo").val() + "©" : wsql;
									
										var waction = f_storageready("sql_statments") == "Incluir" ? "f_insert" : f_storageready("sql_statments") == "Alterar" ? "f_update" : "f_delete";

										$.getJSON('http://www.sisleq.com.br/maitre/admin/sto_processa.php?callback=?', {action: waction, banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"] , sql: wsql}, function(data)
											{
						...
						$('#sto_descricao, #sto_ativo').keypress(function(e)
							{
								if ( e.keyCode == 13 )
									{
										return false;
