...
<html>
    ...
    <body>
        ...
        <div data-role="page" id="relatorio_vendas_mesas">  
				...
				<div class="ui-corner-all custom-corners">
					...
					<div class="ui-body ui-body-a">
						...
						<form method="post">
							...
							<label for="mes_descricao">Mesas : </label>
							<select name="mes_descricao" id="mes_descricao">
			...
			<script>
				...
				$("#relatorio_vendas_mesas").on("pageshow", function() 
					{
						...
						$("#relatorio_vendas_mesas #usu").text( JSON.parse( f_storageready("json_usu") )[0]["usu_nome"] );
						$("#relatorio_vendas_mesas #emp").text( JSON.parse(f_storageready("json_emp"))[0]["emp_descricao"] );

						$.getJSON('http://www.sisleq.com.br/maitre/admin/mes_processa.php?callback=?', {action: "f_select_rel_mes",	banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"]}, function(data)
							{
								var select_mesas = "";
								for(var i = 0, len = data.length; i < len; i++) 
									{
										select_mesas += "<option value='"+data[i]["mes_codigo"]+"'>"+data[i]["mes_descricao"]+"</option>";
									}
								$("#mes_descricao").append(select_mesas).selectmenu("refresh");
							});
						
						$("#relatorio_vendas_mesas").on("focus","input", function()
							{
								$.mobile.silentScroll($(this).offset().top);	
							});
						
						$("#relatorio_vendas_mesas #menuopcoes").on( "click", "li", function() 
							{		
								if( $(this).attr('id') == "Visualizar" )
									{	
										...
										if(ElementsValidation([{"Id" : "rel_periodoinicial", "Caracteres" : "16"}, {"Id" : "rel_periodofinal", "Caracteres" : "16"}], "minlength") == false)
											{
												...
												f_storagewrite("page_redirect" , "relatorio_vendas_mesas.php" );
										...
										var wwhere =  $("#mov_status").val() == "G" ?  " mov.mov_status = 'G' " :  " mov.mov_status = 'C' ";
											wwhere = $("#mes_descricao").val() == "Todas" ? wwhere + "" : wwhere + " and mov.mes_codigo = " + ElementsAspa($("#mes_descricao").val());
										...
										$.getJSON('http://www.sisleq.com.br/maitre/admin/mes_processa.php?callback=?', {action: "f_print_rel_mes",banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"], par1: wfield, par2: wwhere, par3: gropy, par4:orderby }, function(data)
											{
												if( data.length == 0  ) 
													{												
												...
												else 
													{
														...
														$.getJSON('http://www.sisleq.com.br/maitre/admin/mes_processa.php?callback=?', {action:"f_datahora"}, function(data)
															{
																f_storagewrite("rel", "ven" );
																f_storagewrite("interaction", "SS action108" );
																f_storagewrite("fil", "Mesas(s) : " + $("#mes_descricao option:selected").text() + "\n" +
																...
																f_storagewrite("rel_visualizarparametros" ,  "Mesas : " + $("#mes_descricao option:selected").text() + ", <br />" +
