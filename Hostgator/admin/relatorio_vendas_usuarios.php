...
<html>
    ...
    <body>
        ...
        <div data-role="page" id="relatorio_vendas_usuarios">  
				...
				<div class="ui-corner-all custom-corners">
					...
					<div class="ui-body ui-body-a">
						...
						<form method="post">
							...
							<label for="gar_descricao">Garçons : </label>
							<select name="gar_descricao" id="gar_descricao">
							...
							<input type="text" name="rel_periodoinicial" id="rel_periodoinicial" maxlength="16" placeholder="dd/mm/aaaa hh:mm" value="01/01/2017 00:00"  data-wrapper-class="ui-custom"/>
							
							<label for="rel_periodofinal">Período Final : </label>
							<input type="text" name="rel_periodofinal" id="rel_periodofinal" maxlength="16" placeholder="dd/mm/aaaa hh:mm" value="31/12/2017 00:00"  data-wrapper-class="ui-custom" />
			...
			<script>
				...
				$("#relatorio_vendas_usuarios").on("pageshow", function() 
					{
						...
						$("#relatorio_vendas_usuarios #usu").text( JSON.parse( f_storageready("json_usu") )[0]["usu_nome"] );
						$("#relatorio_vendas_usuarios #emp").text( JSON.parse(f_storageready("json_emp"))[0]["emp_descricao"] );
						
						$.getJSON('http://www.sisleq.com.br/maitre/admin/usu_processa.php?callback=?', {action: "f_select_rel_usu",	banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"]}, function(data)
							{
								var select_usuarios = "";
								for(var i = 0, len = data.length; i < len; i++) 
									{
										select_usuarios += "<option value='"+data[i]["usu_nome"]+"'>"+data[i]["usu_nome"]+"</option>";
									}		
								$("#gar_descricao").append(select_usuarios).selectmenu("refresh");
							});
						
						
						$("#relatorio_vendas_usuarios").on("focus","input", function()
							{
								$.mobile.silentScroll($(this).offset().top);	
							});
						
						$("#relatorio_vendas_usuarios #menuopcoes").on( "click", "li", function() 
							{		
								if( $(this).attr('id') == "Visualizar" )
									{	
										...
										if(ElementsValidation([{"Id" : "rel_periodoinicial", "Caracteres" : "16"}, {"Id" : "rel_periodofinal", "Caracteres" : "16"}], "minlength") == false)
											{
												...
												f_storagewrite("page_redirect" , "relatorio_vendas_usuarios.php" );
										...
										if($("#ordem_vendas").val() == "Rec") 
											{
												if($("#mov_status").val() == "G") {
													wfield = " SUBSTR(DATE_FORMAT(mov.mov_datahorapagamento, '%d/%m/%Y %H:%i:%s'), 1,5) as mov_datahorapagamentovenda, ";
													gropy = " mov.mov_datahorapagamento ";
												}
												else {
													wfield = " SUBSTR(DATE_FORMAT(mov.mov_datahoravenda, '%d/%m/%Y %H:%i:%s'), 1,5) as mov_datahorapagamentovenda, ";
													gropy = "mov.mov_datahoravenda";
												}
											}
										else
											{
										...
										var wwhere =  $("#mov_status").val() == "G" ?  " mov.mov_status = 'G' " :  " mov.mov_status = 'C' ";
											wwhere = $("#gar_descricao").val() == "Todos" ? wwhere + "" : wwhere + " and mov.usu_nome = " + ElementsAspa($("#gar_descricao").val());
										...
										$.getJSON('http://www.sisleq.com.br/maitre/admin/usu_processa.php?callback=?', {action: "f_print_rel_usu",banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"] , par1: wfield, par2:wwhere, par3:gropy, par4:orderby }, function(data)
											{
												if( data.length == 0  ) 
													{												
												...
												else 
													{
														...
														$.getJSON('http://www.sisleq.com.br/maitre/admin/usu_processa.php?callback=?', {action:"f_datahora"}, function(data)
															{
																f_storagewrite("rel", "ven" );
																f_storagewrite("interaction", "SS action108" );
																f_storagewrite("fil", "Garçon(s) : " + $("#gar_descricao option:selected").text() + "\n" + 
																...
																f_storagewrite("rel_visualizarparametros" ,"Garçons : " + $("#gar_descricao option:selected").text() + "<br />" +
