				...
				$("#pagamento_itens").on("pageshow", function() 
					{
						...
						$.getJSON('http://www.sisleq.com.br/maitre/operacional/processa_operacional.php?callback=?', {action : "f_select", banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"] ,sql : "pagamento_mesas_itens", par1: f_storageready("mes_codigo")}, function(data) 
							{
										...
										$.each($("#list_PagamentoItens input[type=checkbox]:not(#checktodos)"), function(indice, value)
											{
												if($(this).is(":checked") == true)
													{ 
														wmov_codigo += $(this).siblings("label").attr("id") + "©";
														//wseparador = ;				
													}
											});
										if(wmov_codigo == "") 
