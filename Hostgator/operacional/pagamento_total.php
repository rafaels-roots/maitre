				...
				<div class="ui-corner-all custom-corners">
					...
					<div class="ui-body ui-body-a">
						...
						<form method="post" >
							...
							<input type="text" class="mov_subtotal" id="mov_subtotal" disabled data-wrapper-class="ui-custom"/>
									
							<label for="par_garconcomissao">Comissão  </label>
							<input type="text" class="par_garconcomissao" id="par_garconcomissao" disabled data-wrapper-class="ui-custom" />
							
							<label for="mov_total">Total  </label>
							<input type="text" class="mov_total" id="mov_total" style="font-weight:bold;" disabled data-wrapper-class="ui-custom" />
							
							<label for="mov_valorrecebido">Valor Recebido  </label>
							<input type="text" class="mov_valorrecebido" id="mov_valorrecebido" data-wrapper-class="ui-custom" />  
						
						
							<label for="mov_valortroco">Troco  </label>
							<input type="text" class="mov_valortroco" id="mov_valortroco" style="font-weight:bold;" disabled data-wrapper-class="ui-custom" />  
				...
				$("#pagamento_total").on("pageshow", function() 
					{
						...
						if(f_storagecheck("json_fpg") == true) 
							{
								f_select_fpg( JSON.parse( f_storageready("json_fpg") ));
							}
						else 
							{
								$.getJSON('http://www.sisleq.com.br/maitre/operacional/processa_operacional.php?callback=?', {action: "f_select",	banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"],	sql: "fpg"}, function(data) 
									{
										if( data.length == 0 )
											{  
										...
										if( $(this).val() != "" ) 
											{
												...
												wsql += json_fpg[i]["fpg_codigo"] +"®"+ $(this).val() + "©";
											}	
												
									});
								if(InputsValue == $("#mov_total").val()) 
									{
										$.getJSON('http://www.sisleq.com.br/maitre/operacional/processa_operacional.php?callback=?', {action: "f_pagamento",banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"],mes : f_storageready("mes_codigo"),usu : JSON.parse( f_storageready("json_usu") )[0]["usu_nome"],cod : f_storageready("pagamento_movcodigo"),sql: wsql }, function(data) 
											{	
