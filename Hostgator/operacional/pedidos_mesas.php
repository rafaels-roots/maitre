				...
				$("#pedidos_mesas").on("pageshow", function()
					{
						...
						$.getJSON('http://www.sisleq.com.br/maitre/operacional/processa_operacional.php?callback=?', {action : "f_select", banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"], sql: "pedidos_mesas", par1 : JSON.parse(f_storageready("json_par"))[0]["par_garconexclusivo"], par2: JSON.parse( f_storageready("json_usu") )[0]["usu_nome"] }, function(data)
							{
								if(data.length == 0 )
									{  
						...
						$("#pedidos_mesas #list_PedidosMesas").on( "click", "li", function() 
							{
								...
								$.getJSON('http://www.sisleq.com.br/maitre/operacional/processa_operacional.php?callback=?', { action : "f_select", banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"], sql: "mesa_aberta_fechada", par1 : $(this).children("a").attr("id") }, function(data)
									{ 
										var wmes_aberta = "";
										if( data.length == 0 )
											{  
