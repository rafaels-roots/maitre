				...
				$("#permutar_itens").on("pageshow", function()
					{
						...
						$.getJSON('http://www.sisleq.com.br/maitre/operacional/processa_operacional.php?callback=?', {action : "f_select", banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"] , sql : "permutar_mesas_itens" , par1: f_storageready("mes_codigo")  } , function(data) 
							{
								var json = data;
								f_storagewrite("json_mov_permutar", JSON.stringify(data));
								$.getJSON('http://www.sisleq.com.br/maitre/processa_operacional.php?callback=?', { action : "f_select", banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"], sql :"mesas_destino" , par1: f_storageready("mes_codigo") }, function(data) 
									{
										...
										for(var i = 0, len = json.length; i < len; i++) 
											{						
												var vst = json[i]["mov_print"] == "S" ? "checked" : "";
												var pdz = json[i]["mov_produzido"] == "S" ? "checked" : "";
										
												list_PermutarItens += "<li>" +
																	"<a href='#' class='ui-btn' style='padding:0;'>" +
																		"<div>" + 
																			"<label class='custom-label'>"+json[i]["mov_qtde"]+ " - " + json[i]["pro_descricao"] + "<input type='checkbox' /></label>" + 
																		"</div>" +
																		"<p style='margin-left:.6em;font-size:.95em;'>Observação : "+json[i]["mov_observacao"]+"</p>" + 
																	"</a>" +
						...
						$("#ok_Permutar").on("click", function()
							{
								$.mobile.loading("show");										
								$.getJSON('http://www.sisleq.com.br/maitre/operacional/processa_operacional.php?callback=??', {action : "f_select", banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"],sql :"garcon_destino", par1: $("#mes_destino").val() }, function(data) 
									{
										var wgarcon = "";
										if( data.length == 0 )
											{  
										...
										var wsql = "";
										var json_mov_permutar = JSON.parse(f_storageready("json_mov_permutar"));
										var mes_codigo = f_storageready("mes_codigo");		
										$.each($("#list_PermutarItens a input[type=checkbox]:not(#checktodos)"), function(indice, value)
											{
												if($(this).is(":checked") == true)
													{ 
														wsql += json_mov_permutar[indice]["pro_codigo"] +"®"+		
																json_mov_permutar[indice]["mov_qtde"] +"®"+											
																json_mov_permutar[indice]["mov_valorunitario"] +"®"+						
																json_mov_permutar[indice]["mov_datahoravenda"] +"®"+
																json_mov_permutar[indice]["mov_datahoradespacho"] +"®"+														
																json_mov_permutar[indice]["mov_status"] +"®"+							
																json_mov_permutar[indice]["mov_print"] +"®"+
																json_mov_permutar[indice]["mov_produzido"] +"®"+
																json_mov_permutar[indice]["mov_observacao"] +"®"+	
										...
										$.getJSON('http://www.sisleq.com.br/maitre/operacional/processa_operacional.php?callback=?', {action: "f_permutarpedidos",banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"],mes : $("#mes_destino").val(),usu : wgarcon ,sql: wsql }, function(data) 
											{
