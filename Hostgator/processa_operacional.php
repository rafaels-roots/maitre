...
function f_insertpedidos()
{
	...
	if (! $wbancoopen ) 
	{ 
		echo json_encode ( array ('value' => 'sql_error_open')  ); 
		return true;
	}
	...
	if (! f_bancocharset($wbancoopen,'utf8') )
	{
		...
		echo json_encode ( array ('value' => 'sql_error_charset') );
		return true;
	}
	...
	{
		mysqli_close($wbancoopen);
		echo json_encode ( array ('value' => 'sql_error_autocommit') );
		return true;
	}
	...
	for($x = 0; $x < $size; $x++) 
	{
		...
		{ 
			...
			mysqli_close($wbancoopen);
			echo json_encode ( array ('value' => 'sql_error_command') );
			return true;
	...
	if (! mysqli_commit($wbancoopen))
	{
		...
		{
			mysqli_close($wbancoopen);
			echo json_encode ( array ('value' => 'sql_error_rollback') );
			return true;
		}
		else
		{
			mysqli_close($wbancoopen);
			echo json_encode ( array ('value' => 'sql_error_commit') );
			return true;
		}	
	}
	mysqli_close($wbancoopen);
...
function f_permutarpedidos()
{
	...
	for($x = 0; $x < $size; $x++) 
	{
		$_sql = !($x % 2) ? 'insert into mov '.
							...
							')'
							: 
		                    'delete from mov where mov_status in ("P","D") and ' . $wsql[$x] ;
