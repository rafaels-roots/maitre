				...
				<div class="ui-corner-all custom-corners">
					...
					<div class="ui-body ui-body-a">
						...
						<form method="post" >
							...
							<input type="number" name="fpg_codigo" id="fpg_codigo" disabled>
							
							<label for="fpg_descricao">Descrição: </label>
							<input type="text" id="fpg_descricao" name="fpg_descricao" />
				...
				$("#crud_formas").on("pageshow", function() 
					{
						...
						ElementsValidation( [ {"Id" : "fpg_descricao", "Caracteres" : "30"}], "maxlength" );
								...
								if($(this).attr('id') == "Gravar") 
									{
										...
										var wsql = f_storageready("sql_statments") == "Incluir" ? "insert into fpg (fpg_descricao, fpg_ativa) values ("+ 
																									 ElementsAspa($("#fpg_descricao").val()) + "," +
																									 ElementsAspa($("#fpg_ativa").val()) + ")" : "" ; 
																		
										wsql = f_storageready("sql_statments") == "Alterar" ? "update fpg set fpg_descricao = " + ElementsAspa($("#fpg_descricao").val()) + "," +
																					"fpg_ativa = " + ElementsAspa($("#fpg_ativa").val()) + 
																					" where fpg_codigo =  " + ElementsAspa($("#fpg_codigo").val()) : wsql;
																					
										wsql = f_storageready("sql_statments") == "Excluir" ? "delete from fpg where fpg_codigo = " + ElementsAspa($("#fpg_codigo").val()) + 
																								" and not fpg_codigo in (select mov.fpg_codigo from mov mov) " : wsql;
