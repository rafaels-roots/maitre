				...
				<div class="ui-corner-all custom-corners">
					...
					<div class="ui-body ui-body-a">
						...
						<form method="post" >
							...
							<input type="number" name="imp_codigo" id="imp_codigo" disabled>
							
							<label for="imp_ip">IP :  </label>
							<input type="text" name="imp_ip" id="imp_ip" >
							
							<label for="imp_porta">Porta :  </label>
							<input type="number" name="imp_porta" id="imp_porta" >
							
							<label for="imp_local">Local : </label>
							<input type="text" id="imp_local" name="imp_local" />
							...
							<input type="number" id="imp_avancorodape" name="imp_avancorodape" />
				...
				$("#crud_impressoras").on("pageshow", function() 
					{
						...
						ElementsValidation( [  {"Id" : "imp_local", "Caracteres" : "30"} ,{"Id" : "imp_porta", "Caracteres" : "4"}, {"Id" : "imp_ip", "Caracteres" : "17"}, {"Id" : "imp_avancorodape", "Caracteres" : "3"} ], "maxlength" );
								...
								if($(this).attr('id') == "Gravar") 
									{
										...
										var wsql = f_storageready("sql_statments") == "Incluir" ? "insert into imp (imp_ip, imp_porta, imp_local, imp_ativa, imp_avancorodape) values ("+ 
																									 ElementsAspa($("#imp_ip").val()) + "," +
																									 ElementsAspa($("#imp_porta").val()) + "," +
																									 ElementsAspa($("#imp_local").val()) + "," +
																									 ElementsAspa($("#imp_ativa").val()) + "," + 
																									 ElementsAspa($("#imp_avancorodape").val()) + ")" : "" ; 
																		
										wsql = f_storageready("sql_statments") == "Alterar" ? "update imp set imp_ip = " + ElementsAspa($("#imp_ip").val()) + "," +
																					"imp_porta = " + ElementsAspa($("#imp_porta").val()) + "," +
																					"imp_local = " + ElementsAspa($("#imp_local").val()) + "," +
																					"imp_ativa = " + ElementsAspa($("#imp_ativa").val()) + "," +
																					"imp_avancorodape = " + ElementsAspa($("#imp_avancorodape").val()) + 																					
																					" where imp_codigo =  " + ElementsAspa($("#imp_codigo").val()) : wsql;
																					
										wsql = f_storageready("sql_statments") == "Excluir" ? "delete from imp where imp_codigo = " + ElementsAspa($("#imp_codigo").val()) : wsql;
