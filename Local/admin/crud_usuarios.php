				...
				<div class="ui-corner-all custom-corners">
					...
					<div class="ui-body ui-body-a">		
						<form method="post" >
							...
							<input type="password" id="usu_csenha" name="usu_csenha" />

							<label for="usu_modulos">Módulos de Acesso : [Pedidos/Chef/Caixa/Admin] </label>
							<input type="text" id="usu_modulos" name="usu_modulos" />
				...
				$("#crud_usuarios").on("pageshow", function() 
					{
						...
						ElementsValidation( [ {"Id" : "usu_nome", "Caracteres" : "30"},{"Id" : "usu_senha", "Caracteres" : "15"}, {"Id" : "usu_csenha", "Caracteres" : "15"}, {"Id" : "usu_modulos", "Caracteres" : "30"}], "maxlength" );
								...
								if($(this).attr('id') == "Gravar") 
									{
										...
										var wsql = f_storageready("sql_statments") == "Incluir" ? "insert into usu ( usu_nome, usu_senha, usu_modulos, usu_ativo) values ("+ 
																								 ElementsAspa($("#usu_nome").val()) + "," + 
																								 ElementsAspa($("#usu_senha").val()) + "," +
																								 ElementsAspa($("#usu_modulos").val()) + "," +
																								 ElementsAspa($("#usu_ativo").val()) + ")" : "" ; 
																	
										wsql = f_storageready("sql_statments") == "Alterar" ? "update usu set usu_nome = " + ElementsAspa( $("#usu_nome").val() ) + "," + 
																					"usu_senha = " + ElementsAspa($("#usu_senha").val()) + "," +
																					"usu_modulos = " + ElementsAspa($("#usu_modulos").val()) + "," + 
																					"usu_ativo = " + ElementsAspa($("#usu_ativo").val()) + 
																					" where usu_nome =  " + ElementsAspa($("#usu_nome").val()) : wsql;
									
										wsql = f_storageready("sql_statments") == "Excluir" ? "delete from usu where usu_nome = " + ElementsAspa($("#usu_nome").val()) +
																							  " and usu_nome <> " + ElementsAspa(JSON.parse( f_storageready("json_usu") )[0]["usu_nome"]) : wsql;
