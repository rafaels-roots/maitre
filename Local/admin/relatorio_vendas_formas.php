				...
				$("#relatorio_vendas_formas").on("pageshow", function() 
					{
						...
						var json_select = {action: "f_select",	banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"] ,sql: "select_rel_fpg"};
							...
							{	
								if( $(this).attr('id') == "Visualizar" )
									{	
										...
										var wfield = $("#ordem_vendas").val() == "Rec" || $("#ordem_vendas").val() == "Dia" ? " SUBSTR(DATE_FORMAT(pgt.pgt_datahorapagamento, '%d/%m/%Y %H:%i:%s'), 1,5) as mov_datahorapagamento, " : " SUBSTR(DATE_FORMAT(pgt.pgt_datahorapagamento, '%d/%m/%Y %H:%i:%s'), 4,7) as mov_datahorapagamento, "; 
										...
										var rel_opcoes = {action: "f_select",banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"] ,sql: "print_rel_fpg", par1: wfield, par2 : wwhere, par3: gropy }; 
