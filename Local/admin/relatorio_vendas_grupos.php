...
<html>
    ...
    <body>
        ...
        <div data-role="page" id="relatorio_vendas_grupos">  
				...
				<div class="ui-corner-all custom-corners">
					...
					<div class="ui-body ui-body-a">
						...
						<form method="post">
							...
							<label for="gru_descricao">Grupos : </label>
							<select name="gru_descricao" id="gru_descricao">
							...
							<input type="text" name="rel_periodoinicial" id="rel_periodoinicial" placeholder="dd/mm/aaaa hh:mm" value="01/01/2017 00:00"  />
							
							<label for="rel_periodofinal">Período Final : </label>
							<input type="text" name="rel_periodofinal" id="rel_periodofinal"  placeholder="dd/mm/aaaa hh:mm" value="31/12/2017 00:00" />
			...
			<script>
				...
				$("#relatorio_vendas_grupos").on("pageshow", function() 
					{
						...
						$("#relatorio_vendas_grupos #usu").text( JSON.parse( f_storageready("json_usu") )[0]["usu_nome"] );
						$("#relatorio_vendas_grupos #emp").text( JSON.parse(f_storageready("json_emp"))[0]["emp_descricao"] );
						
						var json_select = {action: "f_select",	banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"] ,sql: "select_rel_gru"};					
						$.getJSON('http://192.168.0.7/maitre/processa_admin.php?callback=?', json_select, function(data)
							{
								var select_grupos = "";
								for(var i = 0, len = data.length; i < len; i++) 
									{
										select_grupos += "<option value='"+data[i]["gru_codigo"]+"'>"+data[i]["gru_descricao"]+"</option>";
									}
								$("#gru_descricao").append(select_grupos).selectmenu("refresh");
								ElementsValidation( [ {"Id" : "rel_periodoinicial", "Caracteres" : "16"},{"Id" : "rel_periodofinal", "Caracteres" : "16"}], "maxlength" );
							});
							
						$("#relatorio_vendas_grupos").on("focus","input", function()
							{
								$.mobile.silentScroll($(this).offset().top);	
							});
							
						$("#relatorio_vendas_grupos #menuopcoes").on( "click", "li", function() 
							{		
								if( $(this).attr('id') == "Visualizar" )
									{	
										...
										if(ElementsValidation([{"Id" : "rel_periodoinicial", "Caracteres" : "16"}, {"Id" : "rel_periodofinal", "Caracteres" : "16"}], "minlength") == false)
											{
												...
												f_storagewrite("page_redirect" , "relatorio_vendas_grupos.php" );
										...
										var rel_opcoes = {action: "f_select",banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"] ,sql: "print_rel_gru", par1: wfield, par2:wwhere, par3:gropy, par4:orderby };
														...
														$.getJSON('http://192.168.0.7/maitre/processa_admin.php?callback=?', {action:"f_datahora"}, function(data)
															{
																...
																f_storagewrite("fil", "Grupo(s) : " + $("#gru_descricao option:selected").text() + "\n" +
																...
																f_storagewrite("rel_visualizarparametros" , "Grupos : " + $("#gru_descricao option:selected").text() + ", <br />" +
