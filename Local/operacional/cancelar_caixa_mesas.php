...
<html>
    ...
    <body>
        ...
        <div data-role="page" id="cancelar_caixa_mesas">  
			...
			<script>
				$("#cancelar_caixa_mesas").on("pageshow", function()
					{
						...
						$("#cancelar_caixa_mesas #usu").text( JSON.parse( f_storageready("json_usu") )[0]["usu_nome"] );
						$("#cancelar_caixa_mesas #emp").text( JSON.parse(f_storageready("json_emp"))[0]["emp_descricao"] );
					
						//var json_mes =  "";
						$.getJSON('http://192.168.0.7/maitre/processa_operacional.php?callback=?', {action : "f_select", banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"], sql : "cancelar_mesas_caixa" }, function(data) 
							{
								//json_mes = data;
								if( JSON.stringify(data).indexOf("sql_record_null") >= 0 )
									...
									{
										//$("#cancelar_caixa_mesas #list_CancelarCaixaMesas li").remove();
										var list_CancelarCaixaMesas = "";
										for(var i = 0, len = data.length; i < len; i++) 
											{						
												list_CancelarCaixaMesas += "<li>" +
																				"<a href='cancelar_itens.php' style='padding-top:0;padding-bottom:0;'>" +
																					"<h2 id='"+data[i]["mes_codigo"]+"'>"+data[i]["mes_descricao"]+"</h2>" +
																					"<p style='font-size:.95em;'>Garçon : "+data[i]["usu_nome"]+"</p>" +
																				"</a>" +
																			"</li>";
											}
										$("#list_CancelarCaixaMesas").append(list_CancelarCaixaMesas).listview("refresh");
									}
							});	
					
						$("#cancelar_caixa_mesas #list_CancelarCaixaMesas").on("click", "li", function() 
							{
