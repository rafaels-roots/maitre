				...
				$("#menu_impressoras").on("pageshow", function() 
					{ 	
						...
						if(f_storagecheck("json_imp") == true) 
							{
								json_imp = JSON.parse(f_storageready("json_imp"));
								f_storagewrite("json_imp", JSON.stringify(json_imp));	
									
							}
						else 
							{
								json_imp = {action: "f_select",	banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"],	sql: "imp"};
								$.getJSON('http://192.168.0.7/maitre/processa_operacional.php?callback=?', json_imp, function(data) 
									{
						...
						var request_populationList = setInterval(function()
														{
															if(f_storagecheck("json_imp") == true) 
																{
																	$("#list_MenuImpressoras li").remove();
																	var list_MenuImpressoras = "";
																	for(var i = 0, len = json_imp.length; i < len; i++) 
																		{	
																			list_MenuImpressoras += "<li>" +
																										"<a href='#' data-rel='popup' data-position-to='window'>" +
																											json_imp[i]["imp_local"]  +
																										"</a>" +
																									"</li>";
																		}
																	$("#list_MenuImpressoras").append(list_MenuImpressoras).listview('refresh');
																	clearInterval(request_populationList);	
																}
														}, 100);
						/*
						f_storageready("rel") == "cda" ? $("#set").text("*** " + JSON.parse(f_storageready("json_stocda"))[0]["sto_descricao"] + " ***") : $("#set").text("*");
						*/ 
