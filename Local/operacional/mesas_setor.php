				...
				$("#mesas_setor").on("pageshow", function()
					{
						...
						$.getJSON('http://192.168.0.7/maitre/processa_operacional.php?callback=?', {action: "f_select",	banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"] ,sql : "mesas_setor",par1: JSON.parse(f_storageready("json_par"))[0]["par_horarioverao"],par2: f_storageready("json_messto")}, function(data) 
							{ 
								json_mes = data;
								if( JSON.stringify(data).indexOf("sql_record_null") == -1 )
									{  
										var mes_descricao = "";
										var count = "";
										var collapsible = "";
								...
								if($(this).attr('id') == "Atualizar")
									{
										...
										var wdados = {action: "f_vistoproduzido",banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"],sql: wsql };
										$.getJSON('http://192.168.0.7/maitre/processa_operacional.php?callback=?', wdados, function() 
											{
