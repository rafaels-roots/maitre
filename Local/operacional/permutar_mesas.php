				...
				$("#permutar_mesas").on("pageshow", function()
					{
						...
						//var json_mes =  "";
						$.getJSON('http://192.168.0.7/maitre/processa_operacional.php?callback=?', {action : "f_select", banco : JSON.parse(f_storageready("json_ter"))[0]["emp_banco"], sql : "permutar_mesas", par1: JSON.parse(f_storageready("json_par"))[0]["par_garconexclusivo"] , par2: ElementsAspa(JSON.parse( f_storageready("json_usu") )[0]["usu_nome"]) }, function(data) 
							{
								//json_mes = data;
								if( JSON.stringify(data).indexOf("sql_record_null") >= 0 )
									...
									{
										//$("#permutar_mesas #list_PermutarMesas li").remove();
